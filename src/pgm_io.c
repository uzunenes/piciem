...
static const char* g_P2_format_ascii = "P2";
static const char* g_P5_format_binary = "P5";
...
static enum e_file_formats g_file_format_flag;
...
read_magic_number(FILE* file_ptr, lpgm_t* pgm)
{
    ...
    if (strcmp(pgm->magic_number, g_P2_format_ascii) != 0 && strcmp(pgm->magic_number, g_P5_format_binary) != 0)
    {
        fprintf(stderr, "%s(): Your image type: [%s], type must be: [%s] or [%s]. \n", __func__, pgm->magic_number, g_P2_format_ascii, g_P5_format_binary);
        return -1;
    }

    if (strcmp(pgm->magic_number, g_P5_format_binary) == 0)
    {
        g_file_format_flag = e_file_formats_P5;
    }
    else if (strcmp(pgm->magic_number, g_P2_format_ascii) == 0)
    {
        g_file_format_flag = e_file_formats_P2;
    }
...
read_pixel_data(FILE* file_ptr, lpgm_t* pgm)
{
    ...
    while (1)
    {
        if (g_file_format_flag == e_file_formats_P2)
        {
            fscanf(file_ptr, "%f", &pgm->im.data[i]);
        }
        else if (g_file_format_flag == e_file_formats_P5)
        {
...
lpgm_file_write(const lpgm_t* pgm, const char* file_name)
{
    ...
    if (g_file_format_flag == e_file_formats_P5)
    {
        file_ptr = fopen(file_name, "wb");
    }
    else if (g_file_format_flag == e_file_formats_P2)
    {
